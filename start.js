...
// Enable file uploads in Express
...
const UserSessionSchema = new mongoose.Schema({
    ...
    expiresAt: { type: Date, required: true, expires: 3600 }, // 1 hour expiry
});
// Create the model
...
const SECRET_KEY = "Jermiah2311#"; // Change this to a strong secret key
...
const typeDefs = gql`
  	scalar Upload  # ✅ Add this line to define the Upload type
	type Token {
	...
	type Mutation {
        ...
        authenticateUser(walletAddress: String!): UserSession
		metaMaskAuth(walletAddress: String!): UserSession
	}
`;

const resolvers = {
  Upload: GraphQLUpload, // ✅ Add Upload scalar type
  ...
  Query: {
	...
	getSession: async (_, { token }) => {
  	...
  	Mutation: {
		createToken: async (_, { name, symbol, description, twitter, telegram, website, image, signedTransaction }, context) => {
			try {
			  // 🔹 Authenticate User - Verify MetaMask Token
			  ...
			  const walletAddress = decoded.walletAddress; // Extract wallet address
	  
			  console.log(`🔐 Authenticated user: ${walletAddress}`);
	  
			  // 🔹 Ensure User Has Enough Balance (100 AMB)
			  const requiredBalance = web3.utils.toWei('100', 'ether'); // 100 AMB
			  ...
			  if (web3.utils.toBN(userBalance).lt(web3.utils.toBN(requiredBalance))) {
				throw new Error("❌ Insufficient balance. You need at least 100 AMB to create a token.");
			  }
			  ...
			  // 🔹 Create Token on Blockchain (User Signs with MetaMask)
			  const tx = factoryContract.methods.createToken(name, symbol, metadataURI, imageURI);
			  const gas = await tx.estimateGas({ from: walletAddress, value: requiredBalance });
	  
			  if (!signedTransaction) {
				// If no signed transaction is provided, return transaction details for signing
				const transaction = {
				  from: walletAddress,
				  to: factoryContract.options.address,
				  gas,
				  value: requiredBalance, // User pays 100 AMB
				  data: tx.encodeABI(),
				};
				...
				return {
				  transaction,
				  message: "✅ Sign this transaction in MetaMask to create your token.",
				};
			  }
			  ...
			  const transactionHashConfirmed = receipt.transactionHash;
			  ...
			  const tokenData = {
				...
				transactionHash: transactionHashConfirmed,
				description,
				imageURI,
				metadataURI,
				twitter,
		...
		async authenticateUser(_, { walletAddress }) {
			if (!web3.utils.isAddress(walletAddress)) {
			  throw new Error("Invalid Ethereum address");
			}
	  
			// Fetch balance using web3.js
		...
		metaMaskAuth: async (_, { walletAddress }) => {
			if (!web3.utils.isAddress(walletAddress)) {
			  throw new Error("Invalid Ethereum address");
			}
	  
			try {
			  // Fetch balance from AirDAO
			  const balanceWei = await web3.eth.getBalance(walletAddress);
			  const balance = web3.utils.fromWei(balanceWei, "ether"); // Convert to AMB
			  ...
			  console.error("Blockchain or Database error:", err);
			  throw new Error("Blockchain or Database error: " + err.message);
			}
		}
	},
};

...
const resolvers = {
  ...
  Mutation: {
    createToken: async (_, { name, symbol, totalSupply, payerPrivateKey, description, twitter, telegram, website }) => {
      try {
        ...
        console.log("Converted totalSupply to number:", supply);
        
        if (isNaN(supply) || supply <= 0) {
          throw new Error("Invalid total supply. Supply must be a positive number.");
        }
        ...
        saveWalletDetails(contractAddress, bondingCurveWallet, associatedBondingCurveWallet);
